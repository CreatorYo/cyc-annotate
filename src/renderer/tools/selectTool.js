function initSelectTool(state, ctx, canvas, helpers) {
  ...
  function resizeSelectedElement(handle, totalDeltaX, totalDeltaY) {
    ...
    if (newWidth < 10) {
      newWidth = 10;
      if (handle.includes("w")) newX = calcBounds.x + calcBounds.width - 10;
    }
    if (newHeight < 10) {
      newHeight = 10;
      if (handle.includes("n")) newY = calcBounds.y + calcBounds.height - 10;
    }
        ...
        if (!state.editingElementId) {
          HANDLE_ANGLES.forEach((handle) => {
            const handleX = Math.round(
              centerX + Math.cos(handle.angle) * handleRadius
            );
            const handleY = Math.round(
              centerY + Math.sin(handle.angle) * handleRadius
            );
