...
function createTooltip(element) {
  ...
  if (!tooltipText || element.hasTooltipListeners) return
...
function initTooltips() {
  ...
  document.addEventListener('click', (e) => {
    hideAllTooltips()
    if (e.target.closest('.stroke-popup, .drawing-tools-popup, .shapes-popup, .custom-color-popup, .wb-title-dropdown, .whiteboard-settings-popup')) {
      setTimeout(() => {
        document.querySelectorAll('[data-tooltip]').forEach(createTooltip)
      }, 50)
    }
