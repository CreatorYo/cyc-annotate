...
function isOverToolbar() {
  ...
  const pad = 25;
  return (
    p.x >= toolbarBounds.x - pad &&
    p.x <= toolbarBounds.x + toolbarBounds.width + pad &&
    p.y >= toolbarBounds.y - pad &&
    p.y <= toolbarBounds.y + toolbarBounds.height + pad
  );
}
...
function startStandbyPolling() {
  ...
  pollingInterval = setInterval(() => {
    const win = deps.getWin?.();
    if (!win || win.isDestroyed() || !deps.getStandbyMode?.()) {
      stopStandbyPolling();
      return;
    }
